...
public:
    ...
    bool GetCredit(const CTransaction& tx, int64_t& nUSC, int64_t& nShadow) const
    {
        nUSC = 0;
        nShadow = 0;
        BOOST_FOREACH(const CTxOut& txout, tx.vout)
...
public:
    ...
    bool GetCredit(int64_t& nCredUSC, int64_t& nCredShadow, bool fUseCache=true) const
    {
        // Must wait until coinbase is safely deep enough in the chain before valuing it
        if ((IsCoinBase() || IsCoinStake()) && GetBlocksToMaturity() > 0)
        {
            nCredUSC = nCredShadow = 0;
            return true;
        }
        ...
        if (!fUseCache || !fCreditSplitCached)
        {
            nCredUSCCached = 0;
            nCredShadowCached = 0;
            pwallet->GetCredit(*this, nCredUSCCached, nCredShadowCached);
            fCreditSplitCached = true;
        };
        
        nCredUSC = nCredUSCCached;
