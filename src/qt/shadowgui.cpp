// Copyright (c) 2014 The UltimateSecureCash developers
    ...
    nWeight(0)
{
    ...
    resize(1280, 720);
    setWindowTitle(tr("USC") + " - " + tr("Client"));
#ifndef Q_OS_MAC
...
void ShadowGUI::createActions()
{
    ...
    aboutAction = new QAction(QIcon(":/icons/shadow"), tr("&About UltimateSecureCash"), this);
    aboutAction->setToolTip(tr("Show information about UltimateSecureCash"));
    aboutAction->setMenuRole(QAction::AboutRole);
    aboutQtAction = new QAction(QIcon(":/trolltech/qmessagebox/images/qtlogo-64.png"), tr("About &Qt"), this);
    aboutQtAction->setToolTip(tr("Show information about Qt"));
    aboutQtAction->setMenuRole(QAction::AboutQtRole);
    optionsAction = new QAction(QIcon(":/icons/options"), tr("&Options..."), this);
    optionsAction->setToolTip(tr("Modify configuration options for UltimateSecureCash"));
...
void ShadowGUI::setClientModel(ClientModel *clientModel)
{
    ...
    if (clientModel)
    {
        ...
        if (mode != NT_FULL)
        {
            ...
            setWindowTitle(tr("USC") + " - " + tr("Wallet") + ", " + sMode);
        };
        ...
        if (clientModel->isTestNet())
        {
            ...
            if(trayIcon)
            {
                trayIcon->setToolTip(tr("USC client") + QString(" ") + tr("[testnet]"));
...
void ShadowGUI::createTrayIcon()
{
    ...
    trayIcon->setToolTip(tr("USC client"));
...
void ShadowGUI::setNumConnections(int count)
{
    ...
    connectionsIcon.setAttribute("data-title", tr("%n active connection(s) to UltimateSecureCash network", "", count));
}
    ...
    QString strMessage =
        tr("This transaction is over the size limit.  You can still send it for a fee of %1, "
          ...
          "Do you want to pay the fee?").arg(
                BitcoinUnits::formatWithUnit(BitcoinUnits::USC, nFeeRequired));
...
void ShadowGUI::dropEvent(QDropEvent *event)
{
    if(event->mimeData()->hasUrls())
    {
        ...
        if (nValidUrisFound)
            ...
            notificator->notify(Notificator::Warning, tr("URI handling"), tr("URI can not be parsed! This can be caused by an invalid UltimateSecureCash address or malformed URI parameters."));
    }
