    ...
    foreach(const SendCoinsRecipient &rcp, recipients)
    {
        ...
        switch(rcp.txnTypeInd)
        {
            ...
            case TXT_ANON_TO_ANON:
                formatted.append(tr("<b>%1</b> SHADOW, ring size %2 to SHADOW %3 (%4)").arg(BitcoinUnits::formatWithUnit(BitcoinUnits::USC, rcp.amount), QString::number(rcp.nRingSize), Qt::escape(rcp.label), rcp.address));
                inputType = 1;
                nAnonOutputs++;
                break;
            case TXT_ANON_TO_USC:
                formatted.append(tr("<b>%1</b> SHADOW, ring size %2 to USC %3 (%4)").arg(BitcoinUnits::formatWithUnit(BitcoinUnits::USC, rcp.amount), QString::number(rcp.nRingSize), Qt::escape(rcp.label), rcp.address));
                inputType = 1;
                break;
            default:
