...
var qrcode = new QRCode("qrcode", {colorDark:'#eab522', colorLight: '#ffffff', correctLevel: QRCode.CorrectLevel.H, width: 220, height: 220,});
    ...
    check: function(field)
    {
        ...
        for (var i = 0, len = value.length; i < len; ++i)
            if (base58.base58Chars.indexOf(value[i]) == -1) {
                el.css("background", "#eab522").css("color", "white");
                return false;
            }
...
var unit = {
    ...
    setType: function(type) {
        ...
        switch(type) {
            ...
            case 2:
                this.name = "uUSC",
                this.display = "&micro;USC";
                break;

            case 3:
                this.name    = "sUSC",
                this.display = "Satoshi";
                break;

            default:
                this.name = this.display = "USC";
        }
    ...
    $("#recipients").append((
        ...
        +  '<div id="recipient[count]" class="recipient"> \
            <div class="flex-right"> \
                ...
                <input id="pay_to[count]" class="pay_to input_box" title="The address to send the payment to  (e.g. SXywGBZBowrppUwwNUo1GCRDTibzJi7g2M)" placeholder="Enter an address (e.g. SXywGBZBowrppUwwNUo1GCRDTibzJi7g2M)" maxlength="128" oninput="base58.check(this);" onchange="$(\'#label[count]\').val(bridge.getAddressLabel(this.value));"/> \
            ...
            <div class="flex-left"> \
                ...
                <select id="unit[count]" class="unit button is-inverse has-fixed-icon"  style="margin-left:10px; height:43px; width:100px;" onchange="unit.format(\'#amount[count]\', $(this).val());"> \
                    <option value="0" title="Shadow"                    ' + (unit.type == 0 ? "selected" : "") + '>USC</option> \
                    <option value="1" title="Milli (1 / 1000)"   ' + (unit.type == 1 ? "selected" : "") + '>mUSC</option> \
                    <option value="2" title="Micro (1 / 1000000)"' + (unit.type == 2 ? "selected" : "") + '>&micro;USC</option> \
                    <option value="3" title="Satoshi (1 / 100000000)" ' + (unit.type == 3 ? "selected" : "") + '>Satoshi</option> \
                </select> \
            </div> \
