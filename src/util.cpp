...
#else
    const char* pszModule = "ultimatesecurecash";
#endif
    if (pex)
        return strprintf(
...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\UltimateSecureCash
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\UltimateSecureCash
    // Mac: ~/Library/Application Support/UltimateSecureCash
    // Unix: ~/.ultimatesecurecash
    
    
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "UltimateSecureCash";
#else
    fs::path pathRet;
    ...
    if (pszHome == NULL || strlen(pszHome) == 0)
        ...
        return pathRet / "UltimateSecureCash";
    #else
        // Unix
        return pathRet / ".ultimatesecurecash";
    #endif
...
{
    boost::filesystem::path pathConfigFile(GetArg("-conf", "ultimatesecurecash.conf"));
                    ...
                    map<string, vector<string> >& mapMultiSettingsRet)
{
    ...
    if (!streamConfig.good())
        return; // No ultimatesecurecash.conf file is OK
    ...
    {
        // Don't overwrite existing settings so command line settings override ultimatesecurecash.conf
...
{
    boost::filesystem::path pathPidFile(GetArg("-pid", "ultimatesecurecashd.pid"));
    ...
    if (vTimeOffsets.size() >= 5 && vTimeOffsets.size() % 2 == 1)
    {
        ...
        else
        {
            ...
            if (!fDone)
            {
                ...
                {
                    fDone = true;
                    string strMessage = _("Warning: Please check that your computer's date and time are correct! If your clock is wrong UltimateSecureCash will not work properly.");
                    strMiscWarning = strMessage;
                    LogPrintf("*** %s\n", strMessage.c_str());
                    uiInterface.ThreadSafeMessageBox(strMessage+" ", string("UltimateSecureCash"), CClientUIInterface::BTN_OK | CClientUIInterface::ICON_WARNING);
                }
            }
        }
        if (fDebug) {
